...
class MgmTestParticipantAPI(ModelViewSet):
    ...
    def list(self, request):
        if request.user and request.user.is_superuser:
            ...
            return Response({'status': True, 'output': MgmTestParticipantSerializer(MgmTestParticipant.objects.filter(test_id__subject_id__assigned_teacher=request.user.id),many=True).data}, status=status.HTTP_200_OK)
        elif request.user and request.user.is_student:
            test_info = MgmTestParticipant.objects.filter(pupil_id=request.user.id)
            total_data = test_info.count()
            average = test_info.aggregate(Avg('grade'))
            result_data = MgmTestParticipantSerializer(test_info,many=True).data
            return Response({'status': True, 'output': {"results": result_data, "average_grade": average['grade__avg']} }, status=status.HTTP_200_OK)
